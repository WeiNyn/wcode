...
**Status.** ☑ done — `377bf78`; second-layer **APPROVED**. The TUI markdown parser
runs on `pulldown-cmark` (CommonMark + GFM tables/strikethrough/task lists/
footnotes) via the `Blocks` event walk; `render`'s signature, `wrap`, and the
syntect path are unchanged. Follow-up nits: footnote/setext/entity coverage; a list
nested in a quote loses the quote's `muted` base (edge case).
